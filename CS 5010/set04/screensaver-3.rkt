...
;Data definition
;world: (make-world Rect-info Rect-info Boolean NonNegInt NonNegInt)
...
(define-struct world (rects paused? mousex mousey))
...
;rect-info: (make-rect-info NonNegInt NonNegInt Int Int Boolean)
...
(define (initial-world rand)
  (make-world (list) true 0 0))

(begin-for-test
 (check-equal? (initial-world 1) (make-world (list) true 0 0)))
...
(begin-for-test
  (check-equal? (world-after-tick (make-world (list (make-rect-info 200 100 -12 20 false 10 20)
                                                    (make-rect-info 200 200 23 -14 false 10 20)) true 0 0))
                (make-world (list (make-rect-info 200 100 -12 20 false 10 20)
                                  (make-rect-info 200 200 23 -14 false 10 20)) true 0 0)  "should return same world")
  (check-equal? (world-after-tick (make-world (list (make-rect-info 200 100 -12 20 false 10 20)
                                                    (make-rect-info 200 200 23 -14 false 10 20)) false 0 0))
                (make-world (list (make-rect-info 188 120 -12 20 false 10 20)
                                  (make-rect-info 223 186 23 -14 false 10 20)) false 0 0) "Wrong coordinate update")
  (check-equal? (world-after-tick (make-world (list (make-rect-info 200 100 -12 20 true 10 20)
                                                    (make-rect-info 200 200 23 -14 true 10 20)) false 0 0))
                (make-world (list (make-rect-info 200 100 -12 20 true 10 20)
                                  (make-rect-info 200 200 23 -14 true 10 20)) false 0 0) "Wrong coordinate update")
  (check-equal? (world-after-tick (make-world (list (make-rect-info 200 100 -12 20 true 10 20)
                                                    (make-rect-info 200 200 23 -14 false 10 20)) false 0 0))
                (make-world (list (make-rect-info 200 100 -12 20 true 10 20)
                                  (make-rect-info 223 186 23 -14 false 10 20)) false 0 0) "Wrong coordinate update")
  (check-equal? (world-after-tick (make-world (list (make-rect-info 200 100 -12 20 false 10 20)
                ...
                (make-world (list (make-rect-info 188 120 -12 20 false 10 20)
                                  (make-rect-info 200 200 23 -14 true 10 20)) false 0 0) "Wrong coordinate update")
  (check-equal? (world-after-tick (make-world (list (make-rect-info 200 100 -12 20 false 10 20)
                                                    (make-rect-info 200 200 23 -14 true 10 20)) true 0 0))
                (make-world (list (make-rect-info 200 100 -12 20 false 10 20)
                                  (make-rect-info 200 200 23 -14 true 10 20)) true 0 0) "Wrong coordinate update"))                
...
(define (MAKE_RECT x y vx vy)
  (rect-after-tick (make-rect-info x y vx vy false 10 20)))
...
;Example: (rect-location-red-image (make-rect-info 100 100 20 20 false)) -> image
...
;;testcase
(define RECT-FOR-TEST (make-rect-info 50 50 10 10 false 10 20))
(begin-for-test
...
;Exmaple: (draw-single-red-rect (make-rect-info 120 200 30 -15 false 10 20)) -> image
;Strategy: using template of rectangle;

(define (draw-single-red-rect r)
  (place-image IMAGE-RED-RECTANGLE (rect-info-x r) (rect-info-y r) DRAW-CANVAS))

(define (draw-single-blue-rect r)
  (place-image IMAGE-RECTANGLE (rect-info-x r) (rect-info-y r) DRAW-CANVAS))

;;testcases
(define RECT-FOR-TEST-2 (make-rect-info 120 200 30 -15 false 0 0))

(begin-for-test
  (check-equal? (draw-single-red-rect RECT-FOR-TEST-2)
                (place-image IMAGE-RED-RECTANGLE
                             (rect-info-x RECT-FOR-TEST-2) (rect-info-y RECT-FOR-TEST-2)
                             DRAW-CANVAS))
  (check-equal? (draw-single-blue-rect RECT-FOR-TEST-2)
                (place-image IMAGE-RECTANGLE
                             (rect-info-x RECT-FOR-TEST-2) (rect-info-y RECT-FOR-TEST-2)
                             DRAW-CANVAS))
  (check-equal? (draw-rect-and-location-red RECT-FOR-TEST-2)
                (place-image (rect-location-red-image RECT-FOR-TEST-2)
                             (rect-info-x RECT-FOR-TEST-2) (rect-info-y RECT-FOR-TEST-2)
                             (place-image IMAGE-RED-RECTANGLE (rect-info-x RECT-FOR-TEST-2)
                                          (rect-info-y RECT-FOR-TEST-2)
                                          DRAW-CANVAS))))
  
;draw-rect-and-location-red Rect-info -> image
;INTERP: draw a colored rectangle on canvas
;GIVEN: rectangle
;RETURNS: image containing rectangle (with color), and canvas.
;Exmaple: (draw-rect-and-location-red (make-rect-info 120 200 30 -15 false)) -> image
;Strategy: using template of rectangle;

(define (draw-rect-and-location-red r)
  (place-image (rect-location-red-image r) (rect-info-x r) (rect-info-y r)
               (place-image IMAGE-RED-RECTANGLE (rect-info-x r) (rect-info-y r) DRAW-CANVAS)))
...
(define (recursive-draw-world rects)
  (cond
    [(and (= (length rects) 1) (if-selected (first rects)))
     (place-image (rect-location-red-image (first rects))
                  (rect-info-x (first rects)) (rect-info-y (first rects))
                  (draw-single-red-rect (first rects)))]

    [(and (= (length rects) 1) (not (if-selected (first rects))))
     (place-image (rect-location-blue-image (first rects))
                  (rect-info-x (first rects)) (rect-info-y (first rects))
                  (draw-single-blue-rect (first rects)))]    
    ...
    [(and (> (length rects) 1) (if-selected (first rects)))
     (place-image (rect-location-red-image (first rects))
                  ...
                  (place-image IMAGE-RED-RECTANGLE
                               (rect-info-x (first rects)) (rect-info-y (first rects))
                               (recursive-draw-world (rest rects))))]
...
;Example: (draw-cursor-on-world (make-world (make-rect-info 200 230 19 18 true) (make-rect-info 40 230 19 20 false)
;                                               false 200 220)) -> image
...
(define (draw-cursor-on-world w)
  (place-image (circle 5 "outline" "red") (world-mousex w) (world-mousey w)
               (recursive-draw-world (world-rects w))))

;;testcases
(define CREATE-WORLD (make-world (list (make-rect-info 200 230 19 18 true 10 20) (make-rect-info 40 230 19 20 false 10 20)) false 200 220))
(begin-for-test
  (check-equal? (draw-cursor-on-world CREATE-WORLD)
                (place-image (circle 5 "outline" "red") (world-mousex CREATE-WORLD) (world-mousey CREATE-WORLD)
                             (recursive-draw-world (world-rects CREATE-WORLD)))
                "Inconsistent image"))
...
(define (draw-real-world w)
  (cond
    ...
    [else (recursive-draw-world (world-rects w))]))
...
(define WORLD-1 (make-world (list (make-rect-info 200 230 19 18 true 10 20) (make-rect-info 40 230 19 20 true 10 20)) false 200 220))
(define WORLD-2 (make-world (list (make-rect-info 200 230 19 18 false 10 20) (make-rect-info 40 230 19 20 false 10 20)) false 200 220))
(define WORLD-3 (make-world (list) false 200 220))
...
(begin-for-test
  ...
  (check-equal? (draw-real-world WORLD-2)
                (recursive-draw-world (world-rects WORLD-2))
                "inconsistent image")
  (check-equal? (draw-real-world WORLD-3)
                (recursive-draw-world (world-rects WORLD-3))
                "inconsistent image"))
...
(define (world-after-key-event w ke)
  (cond
    ...
    [(key=? ke "n") (add-rect w)]
    [(if-speed-key ke) (add-speed w ke)]
    [else w]))

;;testcases
(define WORLD-4 (make-world (list (make-rect-info 100 200 1 2 false 10 20) (make-rect-info 105 201 2 3 false 10 20)) false 50 100))
(define WORLD-5 (make-world (list (make-rect-info 100 200 1 2 false 10 20) (make-rect-info 105 201 2 3 false 10 20)) true 50 100))
(define WORLD-6 (make-world (list (make-rect-info 200 150 0 0 false 0 0) (make-rect-info 100 200 1 2 false 10 20) (make-rect-info 105 201 2 3 false 10 20)
                                  ) false 50 100))
(define WORLD-7 (make-world (list (make-rect-info 200 150 0 0 true 0 0) (make-rect-info 100 200 1 2 false 10 20) (make-rect-info 105 201 2 3 false 10 20)
                                  ) false 50 100))
(define WORLD-8 (make-world (list (make-rect-info 200 150 0 -2 true 0 0) (make-rect-info 100 200 1 2 false 10 20) (make-rect-info 105 201 2 3 false 10 20)
                                  ) false 50 100))
(define WORLD-9 (make-world (list (make-rect-info 200 150 0 2 true 0 0) (make-rect-info 100 200 1 2 false 10 20) (make-rect-info 105 201 2 3 false 10 20)
                                  ) false 50 100))
(define WORLD-10 (make-world (list (make-rect-info 200 150 2 0 true 0 0) (make-rect-info 100 200 1 2 false 10 20) (make-rect-info 105 201 2 3 false 10 20)
                                  ) false 50 100))
(define WORLD-11 (make-world (list (make-rect-info 200 150 -2 0 true 0 0) (make-rect-info 100 200 1 2 false 10 20) (make-rect-info 105 201 2 3 false 10 20)
                                  ) false 50 100))
(define WORLD-12 (make-world (list (make-rect-info 34 28 -2 0 false 0 0) (make-rect-info 200 150 -2 0 true 0 0) (make-rect-info 100 200 1 2 false 10 20)
                                   (make-rect-info 105 201 2 3 false 10 20)) false 50 100))
(define WORLD-13 (make-world (list (make-rect-info 34 28 -2 0 false 0 0) (make-rect-info 200 150 -2 2 true 0 0) (make-rect-info 100 200 1 2 false 10 20)
                                   (make-rect-info 105 201 2 3 false 10 20)) false 50 100))

(begin-for-test
  (check-equal? (world-after-key-event WORLD-4 " ")
                WORLD-5 "world should pause")
  (check-equal? (world-after-key-event WORLD-4 "a")
                WORLD-4 "world should not change")
  (check-equal? (world-after-key-event WORLD-4 "n")
...
(define (toggle-pause w)
  (make-world (world-rects w) (not (world-paused? w)) (world-mousex w) (world-mousey w)))

;;if-speed-key: Keyevent -> Boolean
;;GIVEN: Keyevent
;;RETURNS: true if keyevent is one of add-speed keys
;;Strategy: combining simpler functions
(define (if-speed-key ke)
  (or (key=? ke "up") (key=? ke "left") (key=? ke "right") (key=? ke "down")))
...
(define (add-rect-into-list rects)
  (cons (make-rect-info 200 150 0 0 false 0 0) rects))

;;add-speed: World Keyevent -> World
;;GIVEN: a world and keyevent
;;RETURNS: a new world where its rectangles' speed are added accordingly
;;Strategy: using template of world
(define (add-speed w ke)
...
;;testcases
(define WORLD-15 (make-world (list (make-rect-info 25 20 -3 3 true 10 20) (make-rect-info 10 10 -5 -1 false 10 20)) false 10 20))
(define WORLD-16 (make-world (list (make-rect-info 225 220 -6 6 false 10 20) (make-rect-info 25 20 -3 3 true 10 20)
                                   (make-rect-info 10 10 -5 -1 false 10 20)) false 10 20))
...
(define (mouse-up world mx my)
  (make-world (unselect-rects-recursive (world-rects world)) (world-paused? world) mx my))
...
(define (select-rects-recursive rects mx my)
  (cond
    [(empty? rects) rects]
    [(if-inside (first rects) mx my) (cons (case-down (first rects) mx my)
                                           (select-rects-recursive (rest rects) mx my))]
...
(define (mouse-drag w mx my)
  (make-world (mouse-drag-recursive (world-rects w) mx my) (world-paused? w) mx my)) 
